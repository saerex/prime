						...
						if (!clickedOnDest) {
								if (currentMatchingItem != null) {
										if (currentMatchingItem.GetComponent<MatchingItem> ().itemType == MatchingItem.ItemType.SOURCE) {
												currentMatchingItem.GetComponent<Collider2D>().enabled = true;//enable the collider for the current matching item
		...
		private void RayCast2D (Vector3 pos)
		{
				...
				if (rayCastHit2D.collider.tag == "MatchingItem" && ! startDraging) {
						if (!clickedOnDest) {
								...
								currentMatchingItem.GetComponent<Collider2D>().enabled = false;//disable the collider for the current matching item
